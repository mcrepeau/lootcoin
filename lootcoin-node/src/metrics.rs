...
impl Metrics {
    pub fn new() -> Self {
        ...
        registry.register("lootcoin_chain_height", "Current chain height in blocks", chain_height.clone());

        let chain_difficulty: Gauge<f64, AtomicU64> = Gauge::default();
        registry.register("lootcoin_chain_difficulty", "Current mining difficulty in fractional bits", chain_difficulty.clone());

        let avg_block_time_secs: Gauge<f64, AtomicU64> = Gauge::default();
        registry.register("lootcoin_avg_block_time_secs", "Rolling average block time over the last 10 blocks", avg_block_time_secs.clone());

        let secs_since_last_block: Gauge<f64, AtomicU64> = Gauge::default();
        registry.register("lootcoin_secs_since_last_block", "Seconds elapsed since the most recent block", secs_since_last_block.clone());

        let pot_coins: Gauge<f64, AtomicU64> = Gauge::default();
        registry.register("lootcoin_pot_coins", "Current lottery pot balance in coins", pot_coins.clone());

        let circulating_coins: Gauge<f64, AtomicU64> = Gauge::default();
        registry.register("lootcoin_circulating_coins", "Coins in circulation (total supply minus pot)", circulating_coins.clone());

        let total_supply: Gauge<f64, AtomicU64> = Gauge::default();
        registry.register("lootcoin_total_supply", "Total coin supply (circulating plus pot)", total_supply.clone());

        let lottery_wins_total: Family<TierLabel, Counter<u64, AtomicU64>> = Family::default();
        registry.register("lootcoin_lottery_wins", "Cumulative lottery ticket wins by tier", lottery_wins_total.clone());

        let lottery_payouts_coins_total: Family<TierLabel, Counter<u64, AtomicU64>> = Family::default();
        registry.register("lootcoin_lottery_payouts_coins", "Cumulative lottery coins paid out by tier", lottery_payouts_coins_total.clone());

        let fees_collected_total: Counter<u64, AtomicU64> = Counter::default();
        registry.register("lootcoin_fees_collected", "Cumulative transaction fees collected by the network", fees_collected_total.clone());

        let fees_to_miners_total: Counter<u64, AtomicU64> = Counter::default();
        registry.register("lootcoin_fees_to_miners", "Cumulative fees paid directly to block miners (50% of collected)", fees_to_miners_total.clone());

        let fees_to_pot_total: Counter<u64, AtomicU64> = Counter::default();
        registry.register("lootcoin_fees_to_pot", "Cumulative fees added to the lottery pot (50% of collected)", fees_to_pot_total.clone());

        let mempool_size: Gauge<f64, AtomicU64> = Gauge::default();
        registry.register("lootcoin_mempool_size", "Number of transactions currently in the mempool", mempool_size.clone());

        let peer_count: Gauge<f64, AtomicU64> = Gauge::default();
        registry.register("lootcoin_peer_count", "Number of known peers", peer_count.clone());

        let blocks_total: Counter<u64, AtomicU64> = Counter::default();
        registry.register("lootcoin_blocks", "Cumulative number of blocks applied to the main chain", blocks_total.clone());
    ...
    pub fn seed_lottery(&self, tier: &str, wins: u64, coins: u64) {
        let label = TierLabel { tier: tier.to_string() };
        self.lottery_wins_total.get_or_create(&label).inc_by(wins);
        if coins > 0 {
            self.lottery_payouts_coins_total.get_or_create(&label).inc_by(coins);
        }
    }
    ...
    pub fn record_lottery_win(&self, tier: &str, amount: u64) {
        ...
        if amount > 0 {
            self.lottery_payouts_coins_total.get_or_create(&label).inc_by(amount);
        }
    }
