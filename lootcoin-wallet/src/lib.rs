...
/// Derive an ed25519 secret key from a BIP-39 mnemonic.
///
/// Uses the standard BIP-39 → 64-byte seed derivation (PBKDF2-HMAC-SHA512,
/// no passphrase) and takes the first 32 bytes as the ed25519 secret key.
/// This is consistent with SLIP-0010 "direct seed" usage for ed25519.
fn key_from_mnemonic(m: &Mnemonic) -> [u8; 32] {
    let seed = m.to_seed("");
    seed[..32].try_into().expect("slice is 32 bytes")
}
...
impl Wallet {
    ...
    /// POST /transactions. The nonce is generated randomly by core.
    #[wasm_bindgen]
